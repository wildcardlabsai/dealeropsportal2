...
export interface SupportTicket {
  ...
  created_at: string;
  updated_at: string;
  dealers?: { name: string; email?: string | null; phone?: string | null } | null;
...
export function useSupportTickets() {
  ...
  const allTicketsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("support_tickets")
        .select("*, dealers(name, email, phone)")
        .order("last_message_at", { ascending: false });
      if (error) throw error;
      return (data ?? []) as SupportTicket[];
    },
  ...
  const createTicket = useMutation({
    mutationFn: async (input: {
      ...
      // ticket_number is auto-generated by DB trigger
      ...
      if (input.attachments && input.attachments.length > 0) {
        for (const file of input.attachments) {
          const filePath = `${dealerId}/${ticket.id}/${Date.now()}_${file.name}`;
  ...
  const bulkAssign = useMutation({
    mutationFn: async (ticketIds: string[]) => {
      ...
      const { error } = await supabase
        .from("support_tickets")
        .update({ assigned_to_superadmin_user_id: user.id } as any)
        .in("id", ticketIds);
      if (error) throw error;
    },
    onSuccess: () => {
      toast.success("Tickets assigned to you");
      qc.invalidateQueries({ queryKey: ["support-tickets-admin"] });
    },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  const bulkPriority = useMutation({
    mutationFn: async ({ ticketIds, priority }: { ticketIds: string[]; priority: string }) => {
      if (!user) throw new Error("Not authenticated");
      const { error } = await supabase
        .from("support_tickets")
  ...
  return { ticketsQuery, allTicketsQuery, createTicket, bulkAssign, bulkStatus, bulkPriority };
}
...
export function useSupportTicketDetail(ticketId: string | undefined) {
  ...
  const postReply = useMutation({
    mutationFn: async (input: { message: string; isInternalNote?: boolean; attachments?: File[] }) => {
  ...
  const updateTicket = useMutation({
    mutationFn: async (updates: Record<string, any>) => {
      ...
      if (updates.status && updates.status !== "CLOSED" && updates.status !== "RESOLVED") {
